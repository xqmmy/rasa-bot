...
var userName = "访客11",
	...
	//$(".layim-chat-main").scrollTo(0, $(".layim-chat-main").scrollHeight);
layui.use('layim', function(layim){
	console.log($('.layim-chat-main ul').height())
	$('.layim-chat-main').scrollTop();
  ...
  //打开一个客服面板
	...
	layim.getMessage({
		...
		id: kfId,
		type: kfType,
		content: "您好，小科，请问您有什么需要帮助？face[可爱] "
	});
	layim.getMessage({
		username: kfName,
		avatar: kfAvatar,
		id: kfId,
		type: kfType,
		content: "def[大家都在问：&为什么需要注册？,不在学籍学籍系统的师生怎么办？,报名之后可以修改吗？,报名时找不到对应学校怎么办？,作品上传成功了没有呢？]"
	});
	//发出消息，接收回复的消息
	layim.on('sendMessage', function (res) {
		...
		$.ajax({
			...
			//https://demo1.ji-ling.com/demo/demo/webhooks/rest/webhook
			url: "",
			type: "POST",
		...
		util.event('lay-active', {
			...
			clearMsg: function () {
				...
				layim.getMessage({
					...
					id: kfId,
					type: kfType,
					content: "您好，我是小科，请问您有什么需要帮助？face[可爱] "
				});
				layim.getMessage({
					username: kfName,
					avatar: kfAvatar,
