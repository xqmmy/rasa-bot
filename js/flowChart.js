...
//初始化区域图的对象
	...
	if(property.haveTool){
		...
		//未加代码：加入绘图工具按钮
		this.$tool.append(
			"<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btndown' id='"+this.$id+"_btn_cursor'><i class='ico_cursor'></i></a>"
			+"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct'><i class='ico_direct'></i></a>"
    );
		if(property.toolBtns&&property.toolBtns.length>0){
			tmp="<span></span>";
			for(var i=0;i<property.toolBtns.length;++i){
				tmp+="<a href='javascript:void(0)' type='"+property.toolBtns[i]+"' id='"+this.$id+"_btn_"+property.toolBtns[i].split(" ")[0]+"' class='GooFlow_tool_btn'><i class='ico_"+property.toolBtns[i]+"'></i></a>";//加入自定义按钮
			}
			this.$tool.append(tmp);
		}
		//加入区域划分框工具开关按钮
		if(property.haveGroup)
			this.$tool.append("<span></span><a href='javascript:void(0)' type='group' class='GooFlow_tool_btn' id='"+this.$id+"_btn_group'><i class='ico_group'></i></a>");
		toolWidth=31;
		this.$nowType="cursor";
		//绑定各个按钮的点击事件
	...
	if(this.$editable){
	  ...
	  this.$lineMove=$("<div class='GooFlow_line_move' style='display:block'></div>");//操作折线时的移动框
...
GooFlow.prototype={
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			/*$("a[type='start round']").addClass('no');
			$("a[type='task']").addClass('ac')*/
		}
